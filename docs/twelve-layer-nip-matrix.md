...
| 4 | State | 2 | 1 | 1 | `SPEC.md` defines suspension epoch, revision CAS, dependency barrier, accepted wake, and replay guards. The narrow reference wait reducer is an I2 tracer bullet, but the general task transition vocabulary is not yet a closed state-transition schema, so the whole layer remains I1. |
