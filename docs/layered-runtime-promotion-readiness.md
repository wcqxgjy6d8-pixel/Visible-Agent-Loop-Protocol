...
| Candidate verification | Pass only when rerun on the exact candidate tree | `scripts/verify-examples.sh`, implemented profile-scoped smoke checks, bundled audits, wheel smoke, and `git diff --check` must pass together; full RFC profile conformance and same-commit CI remain separate publication gates |
